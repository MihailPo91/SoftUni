...
class PizzaDelivery:
    ...
    def make_order(self):
        ...
        self.ordered = True
        result_string += f"You've ordered pizza {self.name} prepared with "

        for n, key in enumerate(self.ingredients):
            if n+1 == len(self.ingredients):
                result_string += f'{key}: {self.ingredients[key]} '
            else:
                result_string += f'{key}: {self.ingredients[key]}, '

        result_string += f"and the price will be {self.price}lv."
