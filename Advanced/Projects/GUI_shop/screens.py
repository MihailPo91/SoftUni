...
def render_register_screen(tk):
    ...
    Button(tk, text='Register', bg='green',command=lambda: on_register()).grid(row=4, column=1)
...
def render_login_screen(tk):
    ...
    def on_login():
        ...
        if result:
            ...
            Label(tk, text='Invalid username or password!', fg='red').grid(row=2, column=1)

    Button(tk, text='Login', bg='green', command=lambda: on_login()).grid(row=3, column=1)
...
def render_products_list_screen(tk):
    ...
    for product in products:
        ...
        Button(tk, text="Buy", command=lambda b=product['id']: buy_product(b)).grid(row=row + 3, column=col)
        col += 1
